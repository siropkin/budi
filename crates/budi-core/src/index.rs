...
use crate::chunking::{
    chunk_text, detect_repo_ecosystems, ecosystem_tags_for_chunk, language_label_for_path,
};
...
impl RuntimeIndex {
    ...
    /// Ecosystems detected from repo-level signals (manifest files, project structure).
    pub fn repo_ecosystems(&self) -> &[String] {
        &self.repo_ecosystems
    }
...
mod tests {
    ...
    fn hint_paths_are_normalized_and_confined_to_repo() {
        ...
        let repo_root = tmp.join("budi-repo");
        assert_eq!(
        ...
        let other = tmp.join("other").join("app.rs");
        assert_eq!(
            normalize_hint_path(&repo_root, other.to_str().unwrap()),
            None
        );
        let inside = repo_root.join("src").join("main.rs");
        assert_eq!(
            normalize_hint_path(&repo_root, inside.to_str().unwrap()),
