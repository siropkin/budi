...
pub struct DaemonState {
    ...
    repo_query_stats: Arc<StdMutex<HashMap<String, QueryStats>>>,
...
impl Default for DaemonState {
    fn default() -> Self {
        Self {
            ...
            repo_query_stats: Default::default(),
...
impl DaemonState {
    ...
    pub async fn query(&self, request: QueryRequest, config: &BudiConfig) -> Result<QueryResponse> {
        ...
        // or conversational follow-ups with no code-specific terms.
        let pre_filter_reason = if pre_filter::is_obviously_non_code(&request.prompt) {
            Some("non-code")
        } else if pre_filter::is_conversational_followup(&request.prompt) {
            Some("conversational")
        } else {
            None
        };
        if let Some(intent_label) = pre_filter_reason {
        ...
        // Update query stats (global + per-repo + per-session).
        ...
        }
        {
            ...
            let rs = repo_stats.entry(request.repo_root.clone()).or_default();
            rs.queries += 1;
            if should_inject {
                rs.injections += 1;
                rs.chars_injected += ctx_len;
            } else {
                rs.skips += 1;
            }
        }
    ...
    /// Returns a snapshot of per-repo query activity counters, or None if no stats for this repo.
    pub fn repo_stats_snapshot(
        &self,
        repo_root: &str,
    ) -> Option<(u64, u64, u64, u64, u64, u64, u64)> {
        let guard = self.repo_query_stats.lock().unwrap();
        guard.get(repo_root).map(|s| {
            (
                s.queries,
                s.injections,
                s.skips,
