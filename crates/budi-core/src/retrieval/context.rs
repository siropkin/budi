...
fn render_merged_card(
    ...
    let anchor = if anchor.len() > 120 {
        let mut truncated: String = anchor.chars().take(117).collect();
        truncated.push_str("...");
        truncated
    } else {
        anchor
...
fn sanitize_evidence_line(raw: &str) -> String {
    ...
    let mut out = if trimmed.len() > 120 {
        let mut truncated = trimmed.chars().take(117).collect::<String>();
        truncated.push_str("...");
        truncated
    } else {
        trimmed.to_string()
