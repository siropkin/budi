...
- **`org` → `workspace` rename across daemon, CLI, and ingest envelope** (#836, #840) — mirrors the cloud-dashboard rename shipped in siropkin/budi-cloud#315 / #320. Local code identifiers, the `--workspace-id` CLI flag, the canonical `workspace_id` TOML key in `~/.config/budi/cloud.toml`, and the ingest wire format (`POST /v1/ingest`, `GET /v1/whoami`, `GET /v1/pricing/active`, `budi cloud reset --format json`) all standardize on `workspace_id`. The legacy `org_id` key, `--org-id` flag, and `org` JSON output keys remain accepted via dual-emit / dual-accept aliases during the deprecation window described in ADR-0083 §2 (Amended 2026-05-15). The legacy aliases are removed in a follow-up once siropkin/budi-cloud#321 ships and one release cycle of mixed-version operation has passed.

### Internal — 8.5.2

- **Code organization: split mega-modules** (#800, #813) — every Rust file in `crates/` that was above 2,500 LOC has been split along its natural internal seams. `analytics/queries.rs` (5,405 LOC) split four ways into `queries/{helpers,summary,breakdowns,dimensions}.rs`; `providers/copilot_chat.rs` (4,335 LOC), `commands/stats.rs` (4,216 LOC), `providers/cursor.rs` (3,128 LOC), `commands/doctor.rs` (2,996 LOC), and `cloud_sync.rs` (2,586 LOC) each promoted their inline `mod tests` block into a sibling `tests.rs` under a directory module. The only file still above the threshold is `analytics/tests.rs` (7,684 LOC), which now carries a `// keep-large:` rationale at the top pointing at #805. Public APIs of all three crates are unchanged — every former top-level re-export is preserved via `pub use` in the new `mod.rs` files.
- **Test organization: move large inline test blocks into sibling files** (#805, #829) — five additional `>1,500 LOC` files whose inline test block was `>40%` of the file (`budi-cli/src/{client,main}.rs`, `providers/copilot_chat/jetbrains.rs`, `pricing/mod.rs`, `analytics/sync.rs`) now back their tests with a sibling `tests.rs` (or, in the pricing case, the canonical-by-existing-reference `pricing_tests.rs`). `main.rs` uses an explicit `#[path = "main/tests.rs"]` override since crate-root modules resolve submodules against `src/`. No production code and no test bodies were modified — only relocated; the same set of tests runs.
- **Test coverage: baseline with `cargo-llvm-cov`** (#804, #824) — `scripts/coverage.sh` wraps `cargo llvm-cov --workspace --summary-only -- --test-threads=1` (single-thread required because two `budi-daemon` worker test modules mutate process-wide env vars under a mutex). A non-blocking `coverage` CI job runs the same script and uploads an LCOV artifact. The published baseline lives in `docs/quality/coverage-baseline-8.5.2.md`: workspace totals are 63.70% region / 65.32% function / 64.11% line, with a per-file table and top-10 gap list each pointing at the filed follow-up issue. CONTRIBUTING.md grew an optional "Coverage" subsection.
- **Test coverage gap follow-ups** (#816/#835, #817/#837, #818/#833, #820/#831, #821/#827, #822/#826, #823/#825) — seven gap issues filed off the coverage baseline. All seven closed in this release: `routes/analytics.rs` 1.27% → 85.61%, `routes/hooks.rs` 25.53% → 81.04%, `routes/pricing.rs` raised to ≥80%, `providers/claude_code.rs` raised to ≥80%, `budi-cli stats/mod.rs` raised to ≥55%, `budi-cli/src/client.rs` raised to ≥65%, `analytics/sync.rs` raised to ≥70% (chunk-bounds + helpers). One coverage gap, `providers/cursor.rs` (#819/#839), was reframed mid-release as a non-mechanical task that depends on ADR-0090 (Cursor usage-API contract) fixtures — that work landed via #839 lifting cursor coverage 49.0% → 75.9%, closing #819 inside the 8.5.2 window after all. No coverage-gap issues remain open against the 8.5.2 milestone.
- **Dead code sweep: `cargo-udeps`, `cargo-machete`, `clippy -W unreachable_pub -W dead_code`** (#806, #838) — workspace-wide hygiene pass for the 8.5.2 release. `cargo machete` and `cargo +nightly udeps --workspace --all-targets` both report clean. `cargo clippy --workspace --all-targets -- -W dead_code -W unreachable_pub` reported 255 `unreachable_pub` items across the binary crates (`budi-cli`, `budi-daemon`) and the workspace-private `budi-core` library; all were mechanically narrowed to `pub(crate)` via `clippy --fix`, since neither crate has external API consumers. The surviving `#[allow(dead_code)]` annotations now carry one-line explanations (added one for `PlatformFamily` in `crates/budi-core/src/legacy_proxy.rs`, whose variants are constructed via cfg-gated branches that look dead per-target but are required for cross-platform tests). One redundant `#[allow(dead_code)]` was dropped from `parse_session_dir_for_tests` in `crates/budi-core/src/providers/copilot_chat/jetbrains.rs` since it is exercised by the sibling test module. No `#[deprecated]` items, no `#[cfg(feature = "...")]` gates, and no unused workspace dependencies were found.
- **CI: enforce `cargo-machete` + `cargo-udeps` on PRs** (#808, #809) — `cargo machete` is now a required `rust-checks` step (cheap, stable toolchain). `cargo +nightly udeps --workspace` runs in a new dedicated `udeps` job with `continue-on-error: true` — visible on the Checks tab but allowed-to-fail while we feel out nightly flakiness; promotion to required is tracked alongside the long-standing `cargo deny` promotion in #409. README's "Running CI checks locally" snippet lists all six checks (fmt, clippy, test, udeps, machete, deny) plus the one-time tool-install command.
- **Knowledge base: reconcile SOUL.md / AGENTS.md / CLAUDE.md / CONTRIBUTING.md** (#803, #815) — clarifies the audience split between the four root documents so each one carries the load it's supposed to. `CLAUDE.md` is preserved purely as a compatibility pointer to `SOUL.md` for tools that still hard-code the filename; no other entry point duplicates the others' content. `AGENTS.md` keeps repo-conventions guidance for any LLM agent regardless of vendor; `SOUL.md` carries product identity, architecture, key files, and dev notes; `CONTRIBUTING.md` keeps the human-developer onboarding surface (PR workflow, local CI, coverage tooling).
- **Knowledge base: ADR index audit, fill numbering gaps, regenerate README** (#801, #814) — every ADR under `docs/adr/` now carries a `Last verified: <date> against commit <sha>` footer, the 0084 and 0085 numbering gaps are filled, and `docs/adr/README.md` is regenerated from the directory listing rather than hand-maintained. ADR-0081 was amended to add the durable rule against new `docs/research/` files (already in MEMORY as part of #472), formally retiring the directory by reference instead of resurrecting it.
- **Knowledge base: ADR-0093 §4 retire-with-#789 annotations on JetBrains Phase 1/2 byte-walkers** (#807, #810) — ADR-0093 now carries a `Retire-with: #789` note on §3 (Xodus `projectName` byte-scan) and §4 (Nitrite `file://` URI byte-walker) so the day Phase 3 (#789) lands with a real MVStore + Java-serialization decoder, the Phase 1/2 scaffolding can be deleted in one pass without re-deriving which call sites are scaffolding vs. shipped. No code change in this release — the byte-walkers stay live exactly as they shipped in 8.4.6/8.5.0.
- **Knowledge base: provider module review pass** (#799, #811) — a written code review covering every file under `crates/budi-core/src/providers/` lives in `docs/review/8.5.2/` (per-provider observations + a short cross-cutting summary). Drives the file-split decisions captured in #800 and the test-coverage gap issues captured in #804.
- **README: GitHub release downloads badge** (#828, #830) — adds the GitHub release-downloads shield next to the existing badges so the README at a glance signals project momentum without requiring a click through to the Releases tab.
- **README: soften "team-only" framing for the cloud dashboard** (#832, #834) — five spots in the README that implicitly excluded solo developers from the cloud product (ecosystem row, comparison table, tagline, dashboard screenshot captions, feature bullet) now describe the dashboard as "cloud" rather than "team" — matches how the cloud onboarding flow has framed itself since v8.5.0.

### Cross-repo lockstep

- **siropkin/budi-cloud#321** — paired cloud-side rename for the `org` → `workspace` work (#836/#840). The cloud must accept both `workspace_id` and the legacy `org_id` keys on `POST /v1/ingest`, return both on `GET /v1/whoami` / `GET /v1/pricing/active`, and migrate its local schema. Coordinated cutover plan documented in #836; the legacy keys stay in place on both sides until ≥ 30 days of mixed-version operation have been observed in ingest logs.
...
- **launchd: respawn `budi-daemon` on clean SIGTERM** (#790, #794) — `~/Library/LaunchAgents/dev.getbudi.budi-daemon.plist` shipped with `KeepAlive.SuccessfulExit=false`, which only restarts the daemon on a non-zero exit. The daemon exits cleanly (code 0) on SIGTERM, sleep/wake, `brew upgrade`, and any panic it drains from — so after any of those events the LaunchAgent stayed permanently in `state = not running` and the CLI's empty-payload fallback silently rendered `$0.00 1d · $0.00 7d · $0.00 30d` until next login. The plist generator now emits plain `<key>KeepAlive</key><true/>` so launchd respawns on any exit; `ThrottleInterval = 10` bounds a hard-crashing daemon. Existing installs pick up the new plist on `budi autostart install` (already overwrites unconditionally before reloading); a refreshed `launchctl kickstart` runs on the next CLI invocation after upgrade.
- **Copilot Chat: silence debug-logs spans and pin the attachments fixture** (#791, #796) — `budi doctor` was flagging `tailer rows / Copilot Chat: advanced 29.0 MB in 30 min but no rows landed` with 33 historical `copilot_chat_unknown_record_shape` warnings. Two sources, neither a real reducer regression: (a) `debug-logs/<uuid>/main.jsonl` was being picked up as a session file even though that subtree is OpenTelemetry span output (shape `["attrs","dur","name","sid","spanId","status","ts","type","v"]`); discovery now skips the entire `debug-logs/` directory. (b) VS Code 1.119+ writes high-volume `kind:1 k=["inputState","attachments"]` mutations carrying DOM-like UI introspection records; the warning emitter now treats them as known no-op shapes rather than unknown-shape noise. A redacted fixture pins both shapes.
- **JetBrains Phase 2: diagnose resolver failures and emit a `cwd` hint** (#788, #793) — on the v8.5.0 smoke-test machine the Phase 2 byte-walker successfully extracted `file:///Users/.../Terraform/readme.md` from a Nitrite store but `sessions.repo_id` stayed `NULL` with no log line explaining why. `resolve_workspace_from_paths` is replaced by `resolve_phase2_workspace`, which returns a `Phase2WorkspaceResolution` carrying the longest-common-prefix path, the resolved repo (when any), and one of `no_paths` / `no_common_prefix` / `no_git_along_chain` / `repo_id_resolver_returned_none` as a `failure_reason`. Every Phase 2 attempt that recovers URIs but fails to produce a `repo_id` now emits a single greppable `tracing::debug!` line (session id, dir, URI count, common prefix, reason). When the resolver returns no repo, the longest-common-prefix path is surfaced on `ParsedMessage.cwd` with `cwd_source = "copilot_chat:jetbrains_phase2_prefix"` so the dashboard has a local-path hint to render against `repo_id = NULL` sessions instead of `(unknown)`.

### Added

- **Pipeline: emit `session_title` tag from `IdentityEnricher`** (#787, #792) — the `SESSION_TITLE` constant existed in `crates/budi-core/src/tag_keys.rs` and was listed in `SESSION_IDENTITY_KEYS`, but no enricher actually emitted a `(message_id, key='session_title', value=...)` row. The v8.5.0 backfill (#785, #786) worked around this by re-walking JetBrains session dirs on every sync tick — heavier than necessary and JetBrains-only. `IdentityEnricher` now emits the tag whenever the parser populates `ParsedMessage.session_title`; `Pipeline::process` dedups it to one assistant message per session via the existing `SESSION_IDENTITY_KEYS` path. `backfill_session_titles` collapses back to the original #779 pure-SQL UPDATE shape. `collect_jetbrains_session_titles` stays available as a one-shot historical-row migration helper but is no longer wired into live sync.

### Cross-repo lockstep

- No cross-repo changes required. The wire contract is unchanged from 8.5.0.

## 8.5.0 — 2026-05-12

8.5.0 closes the v8.4.x JetBrains-as-first-class-host arc with the honest delivery of Phase 2 workspace-path attribution, a backfill that finally promotes the parser-emitted `session_title` tag to `sessions.title`, and a pinned wire contract for the upcoming host-aware VS Code build of the budi-cursor extension. The headline scope shift versus the ticket's original framing: a survey of 98 real Nitrite DBs on the smoke-test machine showed the `NtAgentWorkingSetItem.stringContent` field shape the Phase 2 ticket assumed is not what the plugin actually persists — `WorkingSetItem` documents carry only `created_at` / `uuid` / `last_modified_at`, and the file URIs we *can* see live inside an escape-encoded JSON blob in `NtAgentTurn.stringContent`'s per-turn model-state snapshot. The release ships the partial — extract whatever `file://` tokens the byte scan can find, longest-common-prefix → `.git`-walk → `resolve_repo_id` — and ADR-0093 §4 amendment honestly documents the deferred MVStore + Java-serialization decoder work. The realistic ceiling on the smoke-test machine is +1 session beyond Phase 1 (the speculative ≥12-of-23 bar was not achievable for this data shape); for sessions Phase 2 can't reach, the new title backfill at least promotes the session-type label so the dashboard renders something instead of blank. `api_version` stays at `3`.

### Fixed

- **JetBrains Copilot: Phase 2 byte-walker for Nitrite-only session workspace paths** (#778, #782) — `parse_session_dir` now layers a Phase 2 fallback on top of Phase 1 (#766). `extract_nitrite_workspace_paths` scans every `*.nitrite.db` in the session-dir for `file://` URIs (today written into the `currentFileUri` JSON blob inside `NtAgentTurn.stringContent` by recent Copilot Chat plugin builds), percent-decodes them, dedupes, and feeds the result into `resolve_workspace_from_paths` — which computes the longest common directory prefix and walks upward until a `.git` checkout is reachable. Phase 2 fires only when Phase 1 (`projectName` from the Xodus log) returns nothing, preserving the existing dual-store behaviour. New regression coverage uses a size-preserving redacted Nitrite fixture captured from the smoke-test machine. ADR-0093 §4 amendment documents what shipped vs. deferred — the full `NtAgentWorkingSetItem` MVStore-page-segment decoder remains a future amendment, since 95 of 98 real DBs carry no `file://` token at all and that shape is the bulk of the unresolved-agent-session population.
- **JetBrains Copilot: backfill sessions.title from the session_title tag** (#779, #783) — `backfill_session_titles` grew a `copilot_chat` / `jetbrains` arm. Pure-SQL UPDATE: pulls the most-frequent `session_title` tag per session into `sessions.title`, scoped to `provider='copilot_chat' AND surface='jetbrains'`. Idempotent (the `title IS NULL OR title = ''` predicate becomes empty after the first pass). Unlike the existing `claude_code` / `cursor` arms, this one runs across every historical row (no 30-day window) because the parser-emitted tag is already on every message and the dashboard renders historical sessions. Pairs naturally with #778: any session Phase 2 lights up with a Nitrite-derived workspace gets the resolved `projectName` written into the `session_title` tag, which this backfill then promotes to `sessions.title`; sessions Phase 2 can't reach fall through to the session-type fallback (`chat` / `chat-agent` / `chat-edit` / `bg-agent`) so the column renders something honest rather than blank.

### Added

- **ADR-0086 §3.4 v1.1: `surface` field for the host-aware VS Code build of the extension** (#780, #781) — companion to the upcoming siropkin/budi-cursor#64 change that host-detects between Cursor and VS Code at runtime. The wire contract for `~/.local/share/budi/cursor-sessions.json` now documents an optional `surface` field (`cursor` | `vscode`); the daemon's permissive loader already silently accepts unknown fields, so the schema bump is purely additive — both v1 and v1.1 documents coexist on the same path. ADR-0088 §6 reserves the `cursor-onboarding.json` mirror story (forward-looking — no daemon reader yet) so the extension can ship local-counter state without further ADR churn. New `doctor` regression test pins the contract.

### Cross-repo lockstep

- siropkin/budi-cursor#64 (host-aware surface detection in the extension) can land at any time once the v8.5.0 daemon is in users' hands — the wire contract is now stable. The contract is purely additive; no extension-side change is required to keep working against v8.5.0 daemons.
