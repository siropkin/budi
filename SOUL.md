...
- **Model pricing flows through a single manifest-backed `pricing::lookup`** ([ADR-0091](docs/adr/0091-model-pricing-manifest-source-of-truth.md)): embedded LiteLLM baseline at build time, daily refresh against the upstream manifest (opt out with `BUDI_PRICING_REFRESH=0`), every row tagged `pricing_source` so history is immutable. **Team pricing override** ([ADR-0094](docs/adr/0094-custom-team-pricing-and-effective-cost-recalculation.md)): a sibling `cost_cents_effective` column defaults to the LiteLLM-priced `cost_cents_ingested` but is recomputed when the daemon detects an active org price list pulled from `GET app.getbudi.dev/v1/pricing/active`. Local CLI / statusline / extensions and the cloud dashboard all read `_effective`, so cost matches across surfaces by construction. `_ingested` is never overwritten; ADR-0091's immutability rules continue to govern it without modification.
- **`budi init` never mutates user config on the live path**: it creates the data dir, starts the daemon, installs autostart, and wires the recommended integrations (Claude Code statusline, Cursor extension) idempotently. `--no-integrations` opts out. The proxy-routing era is over — there is no longer an init-time path that touches user shell profiles or agent configs.
...
- **budi-cli** — Thin HTTP client to the daemon. Commands: `init`, `stats`, `sessions`, `status`, `statusline`, `doctor`, `update`, `integrations`, `autostart`, `uninstall`, `cloud`, `pricing`, and the DB admin namespace `db` (`db check [--fix]`, `db import`). Session vitals (Prompt Growth, Cache Reuse, Retry Loops, Cost Acceleration) inline into `budi sessions <id>`; `budi sessions latest` shows the most recent session — the standalone `budi vitals` verb (and its `budi health` alias) was removed in 8.3.14 (#585). The bare `budi migrate` / `budi repair` / `budi import` verbs were removed in 8.3.0 (#428) after shipping as hidden aliases during the 8.2.x window; the `db migrate` / `db repair` verbs collapsed into `db check [--fix]` in 8.3.14 (#586) — use `budi db check --fix` to upgrade or repair drift.
...
- **Legacy proxy residue (upgrade only)**: live traffic no longer flows through a proxy. The proxy-routing era is over. The only remaining proxy-related code scans for 8.0/8.1 residue in shell profiles and agent configs, reports retained `proxy_estimated` history honestly, and lets users remove managed blocks via `budi uninstall` (managed cleanup parity).
...
- `crates/budi-cli/src/commands/init.rs` — `budi init` (daemon + autostart + recommended-integrations + detected-agent output).
...
- **Session health**: four vitals — context growth, cache reuse, cost acceleration, retry loops (currently disabled pending a rebuild on top of the tool-outcome signal). Each vital has green/yellow/red state. New sessions start green; vitals only degrade when there is clear evidence of a problem. Tips are provider-aware via the `ProviderKind` enum (Claude Code → `/compact`/`/clear`, Cursor → "new composer session", Other → neutral). When no session ID is provided, auto-select prefers the latest session with assistant activity, then falls back to session timestamps. Session health surfaces via `budi sessions latest` and the cloud dashboard session detail page (which has a full health panel); the statusline no longer renders a health slot — the legacy `coach` preset now silently maps to `["session", "message"]` for migration (#640).
- **Vitals freshness contract** (#602): `session_health` reads `messages` directly — no rollup-table dependency, so end-to-end latency is bounded by the tailer's notify debounce (≈500 ms) plus one SQLite write. Vitals do **not** update on every turn because each metric has a minimum-sample threshold and operates on rolling windows of *assistant* messages (`session_messages` filters to `role='assistant'`):
  - **`context_drag`** — needs ≥5 assistant messages in the post-compact, dominant-model effective set; growth is measured first 3 vs. last 3.
  - **`cache_efficiency`** — needs ≥4 assistant messages in the recent model run; hit rate is over the last 6.
  - **`cost_acceleration`** — needs ≥6 assistant messages (request-cost fallback) and ≥$0.25 session cost; ratio compares the first vs. second half.
  - **`thrashing`** — permanently N/A in 8.3.x (`load_tool_events` is stubbed pending the rebuild on top of the tool-outcome signal).

  Because thrashing never scores, the overall verdict needs ≥2 of the remaining 3 to fire; otherwise it stays `insufficient_data`. The CLI tip on `insufficient_data` surfaces the assistant message count so a user can tell data IS flowing during the warm-up window — "vitals don't seem to update" usually means "this session is below the minimum-samples threshold for two of the three live vitals".
