...
- **budi-cli** — Thin HTTP client to the daemon. Commands: `init`, `stats`, `sessions`, `status`, `statusline`, `doctor`, `update`, `integrations`, `autostart`, `uninstall`, `cloud`, `pricing`, and the DB admin namespace `db` (`db migrate`, `db repair`, `db import`). Session vitals (Prompt Growth, Cache Reuse, Retry Loops, Cost Acceleration) inline into `budi sessions <id>`; `budi sessions latest` shows the most recent session — the standalone `budi vitals` verb (and its `budi health` alias) was removed in 8.3.14 (#585). The bare `budi migrate` / `budi repair` / `budi import` verbs were removed in 8.3.0 (#428) after shipping as hidden aliases during the 8.2.x window — use `budi db <verb>`.
